...
do
{
if(empty($Username) || empty($Password) || empty($FullName)||empty($Age))
{
    $error_msg="All the fields are required";
    break;
}
$sql="UPDATE doctor".
" SET Username='$Username',Password='$Password',FullName='$FullName',Age='$Age'".
"WHERE ID=$ID";
$result=$conn->query($sql);
if(!$result)
{
...
<body>
        ...
        <h2> <center>Edit Doctor</center></h2>
        <hr>
        <br>
        <?php 
        if(!empty($error_msg))
    ...
    <div class="row mb-3">
        ...
        <div class="col-sm-3 d-grid">
            <a class="btn btn-outline-primary" href="../Find/FindDoctor.php" role="button"> Cancel</a>
        </div>  
        </div>
    </form>
    </div>
